    ...
    async execute(client, message, args, user, userStats) {
        ...
        if (args[0] == "details") {
            ...
            if (fenceType && fenceType != "") {
                fence = await Canvas.loadImage(global.src + `/assets/garden/fences/${fenceType.replaceAll(" ", "")}.png`);
            ...
            if (pathType && pathType != "") {
                path = await Canvas.loadImage(global.src + `/assets/garden/paths/${pathType.replaceAll(" ", "")}.png`);
            }

            // path offset = 88 , 42
            const canvas = Canvas.createCanvas(346, 200 + fenceHeight);
            const context = canvas.getContext('2d');
            const garden = await Canvas.loadImage(global.src + '/assets/garden/GardenBase.png');
            const plot = await Canvas.loadImage(global.src + '/assets/garden/Plot.png');
            const bars = await Canvas.loadImage(global.src + '/assets/garden/Bars.png');
                ...
                for (let i = 0; i < userStats.gardenPlots; i++) {
                    ...
                    if (layer == 0) {
                        ...
                        if (plantData) {
                            ...
                            const plantTex = await Canvas.loadImage(global.src + `/assets/garden/plants/${plantFileName}.png`);
